...
import { Plus, Edit, Trash2, CreditCard, Eye, CheckCircle, Clock, XCircle } from 'lucide-react';
...
const AdminPayments = () => {
  ...
  const handleSubmit = async (methodData) => {
    try {
      const url = editingMethod 
        ? `https://api-inventory.isavralabel.com/user-wedding/api/payment-methods/${editingMethod.id}`
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://api-inventory.isavralabel.com/user-wedding/api/payment-methods/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('admin_token')}`
        }
