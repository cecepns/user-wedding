...
const ServiceDetail = () => {
  ...
  const fetchServiceData = async () => {
    try {
      ...
      const serviceResponse = await fetch(`https://api.kingcreativestudio.my.id/user-wedding/api/services/${id}`);
      const serviceData = await serviceResponse.json();
      setService(serviceData);

      // Fetch service items
      const itemsResponse = await fetch(`https://api.kingcreativestudio.my.id/user-wedding/api/services/${id}/items`);
      const itemsData = await itemsResponse.json();
      setItems(itemsData);

      // Fetch service features
      const featuresResponse = await fetch(`https://api.kingcreativestudio.my.id/user-wedding/api/service-features`);
  ...
  const fetchButtonContent = async () => {
    try {
      const response = await fetch('https://api.kingcreativestudio.my.id/user-wedding/api/content-sections/button_item_detail');
      if (response.ok) {
...
const BookingModal = ({ service, selectedItems, onClose, onOrderSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://api.kingcreativestudio.my.id/user-wedding/api/orders', {
        method: 'POST',
        headers: {
